# DSCI441 Presentation Extension — Added Slides

This file adds **three new slides** to the existing 15-slide deck in:
...
- I added these plots to move the project from score reporting to segment-aware analysis.
...
On this added slide, I focus on a simple question: does the model make the same kind of error for every car? My answer is no. The price-decile plot shows that the model is much more stable in lower and middle price ranges, but the error grows sharply for expensive listings. The top decile reaches about ten thousand dollars in MAE, so a single global RMSE is not enough to describe the whole system.

I also added age-bucket and condition-based plots. These figures show that the model is not equally reliable across all segments. Some groups are much easier to predict than others. For me, this is an important upgrade, because it turns the project from a single-score comparison into a more realistic analysis of where the model is strong and where I should be more careful.
...
我还加入了按车龄和车况划分的图。这些图说明模型在不同分组上的可靠性并不一样，有些群体明显更容易预测，有些则更难。对我来说，这部分升级很重要，因为它让这个项目从单纯比一个分数，变成了更贴近真实应用场景的分析，也就是我能更清楚地知道模型哪里强、哪里要谨慎解释。
...
- I added calibration-style and interaction diagnostics after the original report was finished.
...
This added slide explains how I read the upgraded diagnostics from a deployment point of view. The decile calibration plot is useful because it shows whether mean predictions stay aligned with mean true prices across the market. In my results, the alignment is still fairly good overall, but the extremes are more fragile. That means the model captures the broad structure, but it should not use the same confidence language for every price band.

The heatmap and fuel-based error plots tell a similar story in another form. Some condition and fuel combinations are clearly harder than others. So my conclusion is not that the model fails. My conclusion is that the model needs segment-aware interpretation. If I present this model to a user, I should show the prediction together with a warning about which parts of the market are naturally less stable.

中文：

这一页新增内容主要是从部署和使用角度来解释这些升级后的诊断图。价格分位校准图的作用，是检查在不同价格区间里，预测均值和真实均值是不是还能保持一致。从我的结果来看，整体趋势还是对的，但在两端区间会更脆弱一些。这说明模型抓住了市场的大结构，但我不能对所有价格区间都用同样的信心表达。
...
- `Added GitHub Pages Demo: Static but Shareable`
...
After finishing the original local demo, I added one more deliverable: a GitHub Pages web app. This version is static, so it does not run live inference the way the Streamlit app does. Instead, I use precomputed example cases and fixed result pages. The reason is practical. GitHub Pages is easy to share, easy to review, and easy to keep online with the repository.

For me, this is not just a design extra. It changes how the project can be presented. The local Streamlit app still proves that the pipeline is runnable end to end. The GitHub Pages version makes the same project easier to browse, especially for people who want to see the main results and diagnostics quickly. So together, these two demos make the final package more complete.

中文：

在原本的本地 Demo 完成以后，我又额外增加了一个 GitHub Pages 版本的网站。这个版本是静态的，所以它不像 Streamlit 那样能够做实时推理，而是通过预先计算好的示例和固定的结果页面来展示项目。这样做的原因很实际：GitHub Pages 更容易分享，也更方便别人快速查看。
